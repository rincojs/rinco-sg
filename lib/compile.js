 ...
 */
 m.styl = function (filename, fn, order) {
    ...
    var str = fs.file.read(fs.path.join(config.CSS_DIR, filename)).toString();
    str = m.applyPath(str);
...
m.coffee = function (filename, fn, order) {
    // var name = fs.path.basename(filename);
    var str = fs.file.read(fs.path.join(config.JS_DIR, filename)).toString();
    str = m.applyPath(str);
...
m.img = function (filename, fn, dist) {
    ...
    var str = fs.file.read(fs.path.join(config.IMG_DIR, filename));
