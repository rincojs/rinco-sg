...
m.stylus = function (filename, dist) {
    ...
    var dist = fs.path.join(dist, "/css/") || fs.path.join(config.RELATIVE_PATH, "/public/css/");
...
m.less = function (filename, dist) {
    var name = fs.path.basename(filename);
    var str = fs.file.read(null, fs.path.join(config.RELATIVE_PATH, 'assets/css', name)).toString();
    var dist = fs.path.join(dist, "/css/") || fs.path.join(config.RELATIVE_PATH, "/public/css/");
...
m.coffeescript = function (filename, dist) {
    var name = fs.path.basename(filename);
    var str = fs.file.read(null, fs.path.join(config.RELATIVE_PATH, 'assets/js', name)).toString();
    var dist = fs.path.join(dist, "/js/") || fs.path.join(config.RELATIVE_PATH, "/public/js/");
