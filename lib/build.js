...
m.generateUncss = function () {
    try {
      ...
      sh.echo('Ops,\nYou need to install UNCSS npm module.'.yellow);
      return;
    }
...
m.public = function () {
    fs.dir.read(config.PUBLIC_DIR, function (file) {
        ...
        console.log(file);
        fs.file.cp(fs.path.join(config.PUBLIC_DIR,file), fs.path.join(config.BUILD_DIR, file));
    });
};
...
m.link = function (content) {
    ...
    content = content.toString().replace(/(\<\/head\>)/, function (match, head) {
        var script = ' <link rel="stylesheet" href="' + config.USER_CONFIG.path_build + 'assets/css/style.css">';
        return script + "\n" + head;
    });
    content = content.toString().replace(/(\<\/body\>)/, function (match, body) {
        var script = '<script src="' + config.USER_CONFIG.path_build + 'assets/js/script.js" charset="utf-8"></script>';
        return script + "\n" + body;
    });
    return content;
};
